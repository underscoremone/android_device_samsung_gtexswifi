...
static void read_pskey_response(void* p){
    ...
    for (;;) {
        FD_ZERO(&read_set);
        FD_SET(s_bt_fd, &read_set);

        ret = select(s_bt_fd + 1, &read_set, NULL, NULL, &tv);
...
static int copy_mac_file(char *file_dst, char *file_src){
    int fd_src, fd_dst, file_size, ret, i;
    unsigned char *buffer;
    struct stat file_stat;

    ALOGI("%s %s -> %s", __func__, file_src, file_dst);
    fd_src = open(file_src, O_RDONLY);
    if (fd_src < 0) {
        ALOGI("%s open %s error reason: %s", __func__, file_src, strerror(errno));
        return -1;
    }

    fd_dst = open(file_dst, O_WRONLY|O_CREAT,S_IRUSR|S_IWUSR);
    if (fd_src < 0) {
        ALOGI("%s open %s error reason: %s", __func__, file_src, strerror(errno));
        return -1;
    }
    ...
    /* get file size */
    stat(file_src, &file_stat);
    file_size = file_stat.st_size > (MAC_ADDR_FILE_LEN + 10) ? (MAC_ADDR_FILE_LEN + 10) : file_stat.st_size;

    buffer = (unsigned char *)malloc(file_size);
    memset(buffer, 0, file_size);
    ret = read(fd_src, buffer, file_size);
    if (ret < 0) {
        ALOGI("%s read %s error reason: %s", __func__, file_src, strerror(errno));
        goto error;
    }
    if (ret < file_size) {
        unsigned char * p = buffer;
        int len = file_size;
        ALOGW("%s read len: %d, need: %d", __func__, ret, file_size);
        for (i = 0; i < 10; i++) {
            p += ret;
            len -= ret;
            if (!p || len <=0) {
            ...
            ret = read(fd_src, p, len);
            if (ret < 0 || ret == len) {
                break;
            }
        }
        ALOGW("%s practical buf: %s", __func__, buffer);
    }

    ret = write(fd_dst, buffer, file_size);
    if (ret < 0) {
        ALOGI("%s write %s error reason: %s", __func__, file_dst, strerror(errno));
    }

error:
    free(buffer);
    if (fsync(fd_dst) < 0) {
        ALOGI("%s fsync %s error reason: %s", __func__, file_dst, strerror(errno));
    }
    close(fd_src);
    close(fd_dst);
    chmod(file_dst, 0664);
...
static inline int start_pskey_read_thread(int fd) {
    ...
    if (pthread_create(&pskey_read_th, NULL, (void *)read_pskey_response, NULL) != 0) {
...
int sprd_config_init(int fd, char *bdaddr, struct termios *ti)
{
    ...
    if (start_pskey_read_thread(s_bt_fd) < 0) {
        goto error;
    }

    ALOGI("%s write pskey stream", __func__);
    ret = write(s_bt_fd, buf, PSKEY_PRELOAD_SIZE + PSKEY_PREAMBLE_SIZE);
    free(buf);
    buf = NULL;
