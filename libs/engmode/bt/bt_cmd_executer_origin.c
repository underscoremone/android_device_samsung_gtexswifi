...
static void bteng_adapter_state_changed(bt_state_t state) {
    char dut_mode;
    char *str_on[] = { "enter eut mode ok\n", "bt_status=1\nbsp_BT_test_function#open UTPASS" };
    char *str_off[] = { "exit eut mode ok\n", "bt_status=0\nbsp_BT_test_function#close UTPASS" };
    char *p = NULL;
...
static void bteng_nonsig_test_rx_recv(bt_status_t status, uint8_t rssi, uint32_t pkt_cnt,
    ...
    snprintf(buf, 255, "rssi:%d, pkt_cnt:%d, pkt_err_cnt:%d, bit_cnt:%d, bit_err_cnt:%d", rssi,
             pkt_cnt, pkt_err_cnt, bit_cnt, bit_err_cnt);
