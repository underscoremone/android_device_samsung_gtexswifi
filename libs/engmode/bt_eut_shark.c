...
void nonsig_test_rx_rece_callback(bt_status_t status, uint8_t rssi, uint32_t packets, uint32_t packets_err,uint32_t bits,uint32_t bits_err);
...
{
    snprintf(resp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%s", (BTEUT_BT_MODE_BLE == g_bt_mode ? EUT_BLE_ERROR : EUT_BT_ERROR), "error");
}
...
int bt_testmode_set(bteut_bt_mode bt_mode, bteut_testmode testmode, char *rsp)
{
    ...
    ENG_LOG("ADL entry %s(), bt_mode = %d, g_bt_mode = %d, testmode = %d, g_bteut_testmode = %d", __func__, bt_mode, g_bt_mode, testmode, g_bteut_testmode);
    ...
    switch (testmode)
    {
        case BTEUT_TESTMODE_LEAVE:
        {
            ...
            if (BTEUT_TESTMODE_ENTER_NONSIG == g_bteut_testmode)
            {
                ...
                {
                    if (BTEUT_TXRX_STATUS_TXING == g_bteut_txrx_status || BTEUT_TXRX_STATUS_RXING == g_bteut_txrx_status)
                    {
                        ENG_LOG("ADL %s(), txrx_status is ERROR, txrx_status = %d", __func__, g_bteut_txrx_status);
                        goto err;
                    }
                }
            ...
            else if (BTEUT_TESTMODE_ENTER_EUT == g_bteut_testmode)
            {
                ...
                ENG_LOG("ADL %s(), case BTEUT_TESTMODE_LEAVE: called dut_mode_configure(), ret = %d", __func__, ret);

                if (0 != ret)
                {
                    ENG_LOG("ADL %s(), case BTEUT_TESTMODE_LEAVE: called dut_mode_configure(), ret = %d, goto err", __func__, ret);
                    goto err;
                }

                ENG_LOG("ADL %s(), case BTEUT_TESTMODE_LEAVE: set g_bteut_runing to RUNNING_OFF", __func__);
                g_bteut_runing = BTEUT_EUT_RUNNING_OFF;
            }
            
            bt_uninit_eut();

            ENG_LOG("ADL %s(), case BTEUT_TESTMODE_LEAVE: set g_bteut_testmode to BTEUT_TESTMODE_LEAVE", __func__);
        ...
        case BTEUT_TESTMODE_ENTER_EUT:
        {
            ...
            if (BTEUT_TESTMODE_ENTER_EUT != g_bteut_testmode)
            {
                ...
                if (0 != ret)
                {
                    ENG_LOG("ADL %s(), case BTEUT_TESTMODE_ENTER_NONSIG: called bt_init_eut(g_bteut_bt_enabled) is error, goto err", __func__);
                    goto err;
                }
                ...
                while (i++ < BT_EUT_SLEEP_MAX_COUNT)
                {
                    ...
                    ENG_LOG("ADL %s(), i = %d, g_bteut_bt_enabled = %d", __func__, i, g_bteut_bt_enabled);
                    usleep(100*1000);
                }
                ...
                ENG_LOG("ADL %s(), case BTEUT_TESTMODE_ENTER_EUT: called dut_mode_configure(1), ret = %d", __func__, ret);

                if (0 != ret)
                {
                    ENG_LOG("ADL %s(), case BTEUT_TESTMODE_ENTER_EUT: called dut_mode_configure(1), ret = %d, goto err", __func__, ret);
                    goto err;
                }

                ENG_LOG("ADL %s(), case BTEUT_TESTMODE_ENTER_EUT: set g_bteut_runing to BTEUT_EUT_RUNNING_ON", __func__);
                g_bteut_runing = BTEUT_EUT_RUNNING_ON;
            }
            else
            {
                ENG_LOG("ADL %s(), case BTEUT_TESTMODE_ENTER_EUT: now is ENTER_EUT, error.", __func__);
                goto err;
            }
        }
        break;
        ...
        case BTEUT_TESTMODE_ENTER_NONSIG:
        {
            ...
            {
                ENG_LOG("ADL %s(), case BTEUT_TESTMODE_ENTER_NONSIG: now is ENTER NONSIG, error.", __func__);
                goto err;
            }
            ...
            if (0 != ret)
            {
                ENG_LOG("ADL %s(), case BTEUT_TESTMODE_ENTER_NONSIG: called bt_init_eut() is error, goto err", __func__);
                goto err;
            }
        }
        break;
    ...
    strncpy(rsp, (BTEUT_BT_MODE_BLE == g_bt_mode ? EUT_BLE_OK : EUT_BT_OK), BT_EUT_COMMAND_RSP_MAX_LEN);
    rsp_debug(rsp);
...
int bt_testmode_get(bteut_bt_mode bt_mode, char *rsp)
{
    ...
    snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_TESTMODE_REQ_RET : BT_TESTMODE_REQ_RET), (int)g_bteut_testmode);
...
int bt_address_get(char *rsp)
{
    ...
    snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%s", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_ADDRESS_REQ_RET : BT_ADDRESS_REQ_RET), g_bteut_rx.addr);
...
int bt_channel_get(bteut_cmd_type cmd_type, char *rsp)
{
    ...
    if (BTEUT_CMD_TYPE_TX == cmd_type)
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_TX_CHANNEL_REQ_RET : BT_TX_CHANNEL_REQ_RET), g_bteut_tx.channel);
    }
    else if (BTEUT_CMD_TYPE_RX == cmd_type)
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_RX_CHANNEL_REQ_RET : BT_RX_CHANNEL_REQ_RET), g_bteut_rx.channel);
    }
    else
    {
        goto err;
...
int bt_pattern_get(bteut_cmd_type cmd_type, char *rsp)
{
    ...
    if (BTEUT_CMD_TYPE_TX == cmd_type)
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_TX_PATTERN_REQ_RET : BT_TX_PATTERN_REQ_RET), g_bteut_tx.pattern);
    }
    else if (BTEUT_CMD_TYPE_RX == cmd_type)
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_RX_PATTERN_REQ_RET : BT_RX_PATTERN_REQ_RET), g_bteut_rx.pattern);
    }
    else
    {
        goto err;
...
int bt_pkttype_get(bteut_cmd_type cmd_type, char *rsp)
{
    ...
    if (BTEUT_CMD_TYPE_TX == cmd_type)
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_TX_PKTTYPE_REQ_RET : BT_TX_PKTTYPE_REQ_RET), (int)g_bteut_tx.pkttype);
    }
    else if (BTEUT_CMD_TYPE_RX == cmd_type)
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_RX_PKTTYPE_REQ_RET : BT_RX_PKTTYPE_REQ_RET), (int)g_bteut_rx.pkttype);
    }
    else
    {
        goto err;
...
int bt_txpktlen_get(char *rsp)
{
    ...
    snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_TXPKTLEN_REQ_RET : BT_TXPKTLEN_REQ_RET), g_bteut_tx.pktlen);
...
int bt_txpwr_set(bteut_txpwr_type txpwr_type, unsigned int value, char *rsp)
{
    ...
    ENG_LOG("ADL %s(), g_bteut_testmode = %d, g_bteut_runing = %d", __func__, (int)g_bteut_testmode, (int)g_bteut_runing);
...
int bt_txpwr_get(char *rsp)
{
    ...
    snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d,%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_TXPWR_REQ_RET : BT_TXPWR_REQ_RET), (int)g_bteut_tx.txpwr.power_type, g_bteut_tx.txpwr.power_value);
...
int bt_rxgain_set(bteut_gain_mode mode, unsigned int value, char *rsp)
{
    ...
    ENG_LOG("ADL %s(), g_bteut_testmode = %d, g_bteut_runing = %d, value = %d", __func__, (int)g_bteut_testmode, (int)g_bteut_runing, g_bteut_rx.rxgain.value);
    if (BTEUT_TESTMODE_ENTER_EUT == g_bteut_testmode && BTEUT_EUT_RUNNING_ON == g_bteut_runing)
    {
...
int bt_rxgain_get(char *rsp)
{
    ...
    if (BTEUT_GAIN_MODE_FIX == mode)
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d,%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_RXGAIN_REQ_RET : BT_RXGAIN_REQ_RET), mode, g_bteut_rx.rxgain.value);
    }
    else if (BTEUT_GAIN_MODE_AUTO == mode)
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_RXGAIN_REQ_RET : BT_RXGAIN_REQ_RET), mode);
    }
    else
    {
        goto err;
...
*
********************************************************************/
int bt_tx_set(int on_off, bteut_tx_mode tx_mode, unsigned int pktcnt, char *rsp)
{
    int ret = -1;
    char is_ble = 0;
    char cmd[BT_EUT_COMMAND_MAX_LEN+1] = {0x00};

    ENG_LOG("ADL entry %s(), on_off = %d, tx_mode = %d, pktcnt = %d, g_bt_mode = %d", __func__, on_off, (int)tx_mode, pktcnt, g_bt_mode);
    ...
    if (0 == on_off)
    {
        ...
        {
            ENG_LOG("ADL %s(), g_bteut_status is ERROR, g_bteut_txrx_status = %d", __func__, g_bteut_txrx_status);
            goto err;
        }

        ENG_LOG("ADL %s(), call set_nonsig_tx_testmode(), enable = 0, is_ble = %d, g_bt_mode = %d, the rest of other parameters all 0.", __func__, is_ble, g_bt_mode);
        ...
        {
            ENG_LOG("ADL %s(), called set_nonsig_tx_testmode(), ret is ERROR, ret = %d", __func__, ret);
            goto err;
        }
    }
    else if (1 == on_off)
    {
        ...
        ENG_LOG("ADL %s(), call set_nonsig_tx_testmode(), enable = 1, le = %d, pattern = %d, channel = %d, pac_type = %d, pac_len = %d, pwr_type = %d, pwr_value = %d, pkt_cnt = %d", 
                __func__, is_ble, (int)g_bteut_tx.pattern, g_bteut_tx.channel, g_bteut_tx.pkttype, g_bteut_tx.pktlen, g_bteut_tx.txpwr.power_type, g_bteut_tx.txpwr.power_value, pktcnt);
#ifdef HAS_BLUETOOTH_SPRD
        ret = g_sBtInterface->set_nonsig_tx_testmode(1, is_ble, g_bteut_tx.pattern, g_bteut_tx.channel, g_bteut_tx.pkttype, g_bteut_tx.pktlen, g_bteut_tx.txpwr.power_type, g_bteut_tx.txpwr.power_value, pktcnt);
#endif
        ENG_LOG("ADL %s(), called set_nonsig_tx_testmode(), ret = %d", __func__, ret);
...
int bt_tx_get(char *rsp)
{
    ...
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_TX_REQ_RET : BT_TX_REQ_RET), (int)bt_txrx_status);
    }
    else
    {
...
int bt_rx_set(int on_off, char *rsp)
{
    ...
    if (0 == on_off)
    {
        ...
        ENG_LOG("ADL %s(), call set_nonsig_rx_testmode(), enable = 0, le = 0, the rest of other parameters all 0.", __func__);
#ifdef HAS_BLUETOOTH_SPRD
        ...
        {
            ENG_LOG("ADL %s(), called set_nonsig_rx_testmode(), ret is ERROR, ret = %d", __func__, ret);
            goto err;
        }
    }
    else if (1 == on_off)
    {
        ...
        ENG_LOG("ADL %s(), call set_nonsig_rx_testmode(), enable = 1, le = 0, pattern = %d, channel = %d, pac_type = %d, rxgain_value = %d, addr = %s", 
            __func__, (int)g_bteut_rx.pattern, g_bteut_rx.channel, g_bteut_rx.pkttype, rxgain_value, g_bteut_rx.addr);
#ifdef HAS_BLUETOOTH_SPRD
        ret = g_sBtInterface->set_nonsig_rx_testmode(1, is_ble, g_bteut_rx.pattern, g_bteut_rx.channel, g_bteut_rx.pkttype, rxgain_value, addr);
#endif
        ENG_LOG("ADL %s(), called set_nonsig_rx_testmode(), ret = %d", __func__, ret);
...
int bt_rx_get(char *rsp)
{
    ...
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_RX_REQ_RET : BT_RX_REQ_RET), (int)bt_txrx_status);
    }
    else
    {
...
int bt_rxdata_get(char *rsp)
{
    int ret = -1;
    bteut_txrx_status bt_txrx_status = g_bteut_txrx_status;
    ENG_LOG("ADL entry %s(), rf_status = %d, g_bteut_testmode = %d, g_bteut_runing = %d", __func__, bt_txrx_status, g_bteut_testmode, (int)g_bteut_runing);
    ...
    if (BTEUT_TESTMODE_ENTER_NONSIG == g_bteut_testmode)
    {
        ...
        if (BTEUT_TXRX_STATUS_RXING == bt_txrx_status)
        {
            ...
            if (0 != ret)
            {
                ENG_LOG("ADL %s(), call get_nonsig_rx_data() is ERROR, ret = %d, goto err", __func__, ret);
                goto err;
            }
        }
        else
        ...
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d,%d,%d,%d,%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_RXDATA_REQ_RET : BT_RXDATA_REQ_RET), g_bt_rx_data.error_bits, g_bt_rx_data.total_bits, g_bt_rx_data.error_packets, g_bt_rx_data.total_packets, g_bt_rx_data.rssi);

    }
    else if (BTEUT_TESTMODE_ENTER_EUT == g_bteut_testmode && BTEUT_EUT_RUNNING_ON == g_bteut_runing)
    {
        unsigned char i = 0;
        ...
        if (0 != ret)
        {
            ENG_LOG("ADL %s(), callED dut_mode_send(HCI_DUT_GET_RXDATA) is ERROR, ret = %d, goto err", __func__, ret);
            goto err;
        }
        ...
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%d", (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_RXDATA_REQ_RET : BT_RXDATA_REQ_RET), g_bt_rx_data.rssi);
    }
    rsp_debug(rsp);
...
*
********************************************************************/
void nonsig_test_rx_rece_callback(bt_status_t status, uint8_t rssi, uint32_t pkt_cnt, uint32_t pkt_err_cnt, uint32_t bit_cnt, uint32_t bit_err_cnt)
{
    ENG_LOG("ADL entry %s(), status = %d, rssi = %d, pkt_cnt = %d, pkt_err_cnt = %d, bit_cnt = %d, bit_err_cnt = %d", __func__, status, rssi, pkt_cnt, pkt_err_cnt, bit_cnt, bit_err_cnt);
...
void dut_mode_recv(uint16_t opcode, uint8_t *buf, uint8_t len)
{
    ...
    if(HCI_DUT_GET_RXDATA == opcode && 2 == len) 
    {
        ...
        status = *buf;
        rssi = *(buf+1);
        ENG_LOG("ADL %s(), DUT MODE RECV, RX DATA : status = %d, RSSI = %d", __func__, status, rssi);
...
static void bt_adapter_state_changed(bt_state_t state)
{
    ENG_LOG("ADL entry %s(), ADAPTER STATE UPDATED = %s", __func__, (BT_STATE_OFF == state) ? "OFF" : "ON");
...
static int bt_le_transmitter(void)
{
    ...
    ENG_LOG("ADL %s(), channel = %d, pktlen = %d, pattern = %d", __func__, g_bteut_tx.channel, g_bteut_tx.pktlen, g_bteut_tx.pattern);
    ...
    ENG_LOG("ADL %s(), callED le_test_mode(HCI_LE_TRANSMITTER_TEST_OPCODE), ret = %d", __func__, ret);
