...
#define BT_EUT_RX_ADDR_DEAFULT_VALUE            ("00:00:88:C0:FF:EE") /* addr must be the addr of Master, if not ,It can not receive any packet */
...
typedef struct
{
    ...
    unsigned char is_update; /* all rx data whether is update from chip. this value equals to 1, means that all other datas is valid */
} BTEUT_RX_DATA;
