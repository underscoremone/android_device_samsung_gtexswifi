...
int create_log_dir()
{
	...
	if (-1 == ret && (errno != EEXIST)) {
	    ENG_LOG("mkdir %s failed.error: %s\n", top_logdir,strerror(errno));
	    return -1;
	}
	...
	ENG_LOG("the external storage : %s", external_path);
    } else {
	ENG_LOG("EXTERNAL_STORAGE can't find!!");
    }
...
int eng_write_usb(char* diag_data, int r_cnt, int ser_fd)
{
...
FRAME:
		...
		eng_write_usb(ext_log_data_buf, ext_log_buf_len, ser_fd);     //the start series 0x7e will send to the usb/uart,tools can parse it
	    }
