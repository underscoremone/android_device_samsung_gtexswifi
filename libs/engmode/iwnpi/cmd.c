	...
	if (tb_msg[NLNPI_ATTR_REPLY_STATUS]) {
		if (nla_len(tb_msg[NLNPI_ATTR_REPLY_STATUS]) == 4)
			printf("ret: status %d :end\n", *(int *)
			       nla_data(tb_msg[NLNPI_ATTR_REPLY_STATUS]));
		else
...
TOPLEVEL(start, NULL,
	 NLNPI_CMD_START, 0, npi_set_noarg_cmd, "Start npi thread in CP2.");

TOPLEVEL(stop, NULL,
	 NLNPI_CMD_STOP, 0, npi_set_noarg_cmd, "Stop npi thread in CP2.");
...
TOPLEVEL(set_band, "<NUM>",
	 NLNPI_CMD_SET_BAND, 0, npi_set_band_cmd, "Set band.");
TOPLEVEL(set_pkt_length, "<NUM>",
	 NLNPI_CMD_SET_PKTLEN, 0, npi_set_pkt_length_cmd, "Set band.");
TOPLEVEL(set_rate, "<NUM>",
	 NLNPI_CMD_SET_RATE, 0, npi_set_rate_cmd, "Set rate.");
TOPLEVEL(set_channel, "<NUM>",
	 NLNPI_CMD_SET_CHANNEL, 0, npi_set_channel_cmd, "Set channal num.");
TOPLEVEL(set_tx_power, "<NUM>",
	 NLNPI_CMD_SET_TX_POWER, 0, npi_set_tx_power_cmd, "Set tx power.");

TOPLEVEL(set_rx_count, "<NUM>",
	 NLNPI_CMD_CLEAR_RX_COUNT, 0, npi_set_rx_count_cmd, "Clear rx count.");
TOPLEVEL(set_tx_mode, "<NUM>",
	 NLNPI_CMD_SET_TX_MODE, 0, npi_set_tx_mode_cmd, "Set tx mode.");

TOPLEVEL(set_tx_count, "<NUM>",
	 NLNPI_CMD_SET_TX_COUNT, 0, npi_set_tx_count_cmd,
...
TOPLEVEL(set_mac, "<xx:xx:xx:xx:xx:xx>",
	 NLNPI_CMD_SET_MAC, 0, npi_set_mac_cmd,
...
#define REG_PHY1        2
#define REG_RF          3
static int npi_set_reg_cmd(struct nlnpi_state *state,
			   struct nl_cb *cb,
			   struct nl_msg *msg, int argc, char **argv)
{
	...
	if (tb_msg[NLNPI_ATTR_REPLY_DATA]) {
		if (nla_len(tb_msg[NLNPI_ATTR_REPLY_DATA]) == 12) {
			memcpy(count, nla_data(tb_msg[NLNPI_ATTR_REPLY_DATA]),
			       sizeof(count));
			snprintf(ret_result_buf, WLNPI_RES_BUF_LEN, "ret: reg value: rx_end_count=%d rx_err_end_count=%d fcs_fail_count=%d :end\n",
...
TOPLEVEL(get_pkt_length, NULL,
	 NLNPI_CMD_GET_PKTLEN, 0, npi_get_pkt_length_cmd, "Get packet length.");

TOPLEVEL(get_channel, NULL,
	 NLNPI_CMD_GET_CHANNEL, 0, npi_get_channel_cmd, "Get channal num.");
TOPLEVEL(get_band, NULL,
	 NLNPI_CMD_GET_BAND, 0, npi_get_band_cmd, "Get band.");

TOPLEVEL(get_payload, NULL,
	 NLNPI_CMD_GET_PAYLOAD, 0, npi_get_payload_cmd, "Get payload.");

TOPLEVEL(get_bandwidth, NULL,
	 NLNPI_CMD_GET_BANDWIDTH, 0, npi_get_bandwidth_cmd, "Get bandwidth.");

TOPLEVEL(get_rx_ok, NULL,
	 NLNPI_CMD_GET_RX_OK_COUNT, 0, npi_get_rx_ok_cmd,
	 "Get rx right count.");

TOPLEVEL(get_tx_mode, NULL,
	 NLNPI_CMD_GET_TX_MODE, 0, npi_get_tx_mode_cmd,
	 "Get tx mode.");
/*
...
TOPLEVEL(get_rate, NULL,
	 NLNPI_CMD_GET_RATE, 0, npi_get_rate_cmd, "Get tx rate.");
...
static int npi_get_tx_power_cmd(struct nlnpi_state *state,
				struct nl_cb *cb,
				struct nl_msg *msg, int argc, char **argv)
{
	...
	nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM, print_get_tx_power_result,
		  NULL);
	return 0;
nla_put_failure:
	return -ENOBUFS;
...
TOPLEVEL(get_tx_power, NULL,
	 NLNPI_CMD_GET_TX_POWER, 0, npi_get_tx_power_cmd, "Get tx power.");
...
static int npi_get_mac_cmd(struct nlnpi_state *state,
			   struct nl_cb *cb,
			   struct nl_msg *msg, int argc, char **argv)
{
...
static int print_get_reg_result(struct nl_msg *msg, void *arg)
{
	...
	if (tb_msg[NLNPI_ATTR_REPLY_DATA]) {
		...
		if ((count < 1024) || !(count % 4)) {
			memcpy(data, nla_data(tb_msg[NLNPI_ATTR_REPLY_DATA]),
			       count);
...
static int npi_get_reg_cmd(struct nlnpi_state *state,
			   struct nl_cb *cb,
			   struct nl_msg *msg, int argc, char **argv)
{
