...
static int get_iwnpi_ret_status(void)
{
    ...
    while( !feof(fp) )
    {
        ...
        ENG_LOG("ADL %s(), buf = %s, str1 = %s, str2 = %s", __func__, buf, str1, str2);
...
static int get_iwnpi_rxpktcnt(RX_PKTCNT *cnt)
{
    ...
    ENG_LOG("ADL leveling %s(), rx_end_count = %d, rx_err_end_count = %d, fcs_fail_count = %d", __func__, cnt->rx_end_count, cnt->rx_err_end_count, cnt->fcs_fail_count);
    return ret;
}
...
static int start_wifieut(char *rsp)
{
    ...
    ENG_LOG("ADL %s(), callED system(insmod /system/lib/modules/sprdwl.ko), ret = %d", __func__, ret);
    if(ret < 0)
    {
...
static int wifi_tx_start(char *rsp)
{
    ...
    {
        ENG_LOG("ADL %s(), sin_wave_start is 1, goto err. SIN_WAVE_start is 1",  __func__);
        goto err;
    }
...
ok:
    ...
    ENG_LOG("ADL %s(), case ok: set sin_wave_start & tx_start to 0. return 0", __func__);
...
err:
    ...
    g_wifi_data.tx_start = 0;
    g_wifi_data.sin_wave_start = 0;
    ENG_LOG("ADL %s(), case err: set sin_wave_start & tx_start to 0. return -1", __func__);
    rsp_debug(rsp);
    return -1;
...
{
    ENG_LOG("ADL entry %s(), command_code = %d, mode = %d, pktcnt = %d", __func__, command_code, mode, pktcnt);
...
static int wifi_rx_start(char *rsp)
{
    ...
    ENG_LOG("ADL entry %s(), rx_start = %d, tx_start = %d", __func__, g_wifi_data.rx_start, g_wifi_data.tx_start);
...
static int get_iwnpi_tx_power_ret(int *lva, int *lvb)
{
    ...
    while(!feof(fp))
    {
        ...
        if((NULL != str1) && (NULL != str2))
        {
            ret_cnt = sscanf(buf, "ret: level_a:%d,level_b:%d:end", &level_a, &level_b); /* must be match MARCO STR_RET_RET STR_RET_END */
            ENG_LOG("ADL %s(), ret_cnt = %d", __func__, ret_cnt);
            if (ret_cnt > 0)
            {
                ENG_LOG("ADL %s(), ret = %d, break", __func__, ret);
    ...
    *lvb = level_b;
    ...
    ENG_LOG("ADL leaving %s(), level_a = %d, level_b = %d, ret = %d", __func__, level_a, level_b, ret);
    return ret;
}
...
int wifi_band_set(wifi_band band, char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_band %d > %s", band, TMP_FILE);
...
int wifi_band_get(char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_band > %s", TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
...
int wifi_bandwidth_set(wifi_bandwidth band_width, char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_bandwidth %d > %s", band_width, TMP_FILE);
...
int wifi_bandwidth_get(char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_bandwidth > %s", TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
    ...
    snprintf(rsp, WIFI_EUT_COMMAND_RSP_MAX_LEN, "%s%d", WIFI_BANDWIDTH_REQ_RET, band_width);
    rsp_debug(rsp);
...
int wifi_tx_power_level_set(int tx_power, char *rsp)
{
    ...
    if (tx_power < WIFI_TX_POWER_MIN_VALUE || tx_power > WIFI_TX_POWER_MAX_VALUE)
    {
        ENG_LOG("%s(), tx_power's value is invalid, tx_power = %d", __func__, tx_power);
        goto err;
    }

    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_tx_power %d > %s", tx_power, TMP_FILE);
    ret = system(cmd);
...
int wifi_tx_power_level_get(char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_tx_power > %s", TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
    ...
    snprintf(rsp, WIFI_EUT_COMMAND_RSP_MAX_LEN, "%s%d,%d", WIFI_TX_POWER_LEVEL_REQ_RET, level_a, level_b);
    rsp_debug(rsp);
...
int wifi_pkt_length_set(int pkt_len, char *rsp)
{
    ...
    if (pkt_len < WIFI_PKT_LENGTH_MIN_VALUE || pkt_len > WIFI_PKT_LENGTH_MAX_VALUE)
    {
        ENG_LOG("%s(), tx_power's value is invalid, pkt_len = %d", __func__, pkt_len);
        goto err;
    }

    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_pkt_length %d > %s", pkt_len, TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), call system(%s), ret = %d", __func__, cmd, ret);
...
int wifi_pkt_length_get(char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_pkt_length > %s", TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), call system(%s), ret = %d", __func__, cmd, ret);
    ...
    snprintf(rsp, WIFI_EUT_COMMAND_RSP_MAX_LEN, "%s%d", WIFI_PKT_LENGTH_REQ_RET, pkt_length);
    rsp_debug(rsp);
...
int wifi_tx_mode_set(wifi_tx_mode tx_mode, char *rsp)
{
    ...
    {
        ENG_LOG("%s(), tx_power's value is invalid, tx_mode = %d", __func__, tx_mode);
        goto err;
    }
    ...
    {
        ENG_LOG("ADL %s(), tx_mode is CW, tx_start = %d", __func__, g_wifi_data.tx_start);
        if (1 == g_wifi_data.tx_start)
        {
            snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 tx_stop > %s", TMP_FILE);
            ret = system(cmd);
            ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
        ...
        snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 sin_wave > %s", TMP_FILE);
        ret = system(cmd);
        ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
    ...
    else /* other mode, send value to CP2 */
    {
        ...
        snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_tx_mode %d > %s", tx_mode, TMP_FILE);
        ret = system(cmd);
        ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
...
int wifi_tx_mode_get(char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_tx_mode > %s", TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
    ...
    snprintf(rsp, WIFI_EUT_COMMAND_RSP_MAX_LEN, "%s%d", WIFI_TX_MODE_REQ_RET, tx_mode);
    rsp_debug(rsp);
...
int wifi_preamble_set(wifi_preamble_type preamble_type, char *rsp)
{
    ...
    {
        ENG_LOG("%s(), preamble's value is invalid, preamble_type = %d", __func__, preamble_type);
        goto err;
    }

    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_preamble %d > %s", preamble_type, TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
...
int wifi_preamble_get(char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_preamble > %s", TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
    ...
    snprintf(rsp, WIFI_EUT_COMMAND_RSP_MAX_LEN, "%s%d", WIFI_PREAMBLE_REQ_RET, (int)preamble_type);
    rsp_debug(rsp);
...
int wifi_payload_set(wifi_payload payload, char *rsp)
{
    ...
    {
        ENG_LOG("%s(), payload's value is invalid, payload = %d", __func__, payload);
        goto err;
    }

    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_payload %d > %s", payload, TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
...
int wifi_payload_get(char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_payload > %s", TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
    ...
    snprintf(rsp, WIFI_EUT_COMMAND_RSP_MAX_LEN, "%s%d", WIFI_PAYLOAD_REQ_RET, (int)payload);
    rsp_debug(rsp);
...
int wifi_guardinterval_set(wifi_guard_interval gi, char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_guard_interval %d > %s", gi, TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
...
int wifi_guardinterval_get(char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_guard_interval > %s", TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
    ...
    snprintf(rsp, WIFI_EUT_COMMAND_RSP_MAX_LEN, "%s%d", WIFI_GUARDINTERVAL_REQ_RET, (int)gi);
    rsp_debug(rsp);
...
*   ----------------------------
*   para            IN/OUT      type                    note      
*   on_off          IN          int                     on or off mac filter function
*   mac             IN          const char*             mac addr as string format
*   rsp             OUT         char *                  response result
*   ----------------------------------------------------
*   return
*   0:exec successful
...
int wifi_mac_filter_set(int on_off, const char *mac, char *rsp)
{
    ...
    if (1 == on_off)
    {
        ...
        {
            ENG_LOG("%s(), mac's length is invalid, len = %d", __func__, strlen(mac));
            goto err;
        }
        ...
        snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_macfilter %d > %s", on_off, TMP_FILE);
        ...
        snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 set_mac %s > %s", mac_str, TMP_FILE);
...
*   ---------------------------
*   description: get Chip's mac filter switch and mac addr if on, by iwnpi command get_macfilter
*   ----------------------------
*   para        IN/OUT      type            note      
*   rsp         OUT         char *          response result
*   ----------------------------------------------------
...
int wifi_mac_filter_get(char *rsp)
{
    ...
    snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_macfilter > %s", TMP_FILE);
    ret = system(cmd);
    ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);
    ...
    if (1 == on_off)
    {
        ...
        snprintf(cmd, WIFI_EUT_COMMAND_MAX_LEN, "iwnpi wlan0 get_mac > %s", TMP_FILE);
        ret = system(cmd);
        ENG_LOG("ADL %s(), called system(%s), ret = %d", __func__, cmd, ret);

        ret = get_iwnpi_string_ret(STR_RET_MAC_VALUE, STR_RET_END, mac_str, WIFI_MAC_STR_MAX_LEN);
        if(WIFI_INT_INVALID_RET == ret)
        {
            ENG_LOG("iwnpi wlan0 lna_status cmd, err_code = %d", ret);
            goto err;
        ...
        snprintf(rsp, WIFI_EUT_COMMAND_RSP_MAX_LEN, "%s%d,\"%s\"", WIFI_MAC_FILTER_REQ_RET, on_off, mac_str);
    }
    else if (0 == on_off)
    {
        snprintf(rsp, WIFI_EUT_COMMAND_RSP_MAX_LEN, "%s%d", WIFI_MAC_FILTER_REQ_RET, on_off);
    }
    else
    {
        ENG_LOG("%s(), on_off's value is invalid, on_off = %d", __func__, on_off);
