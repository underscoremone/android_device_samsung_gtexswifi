...
#define ENG_PCBA_SUPPORT_CONFIG "/productinfo/PCBA.conf"
#define ENG_BBAT_SUPPORT_CONFIG "/productinfo/BBAT.conf"
...
static char eng_modemdb_diag_buf[ENG_MODEMDB_DIAG_SIZE];
...
static int read_modem_db_attr(char *buffer)
{
	...
	if((fread(&dbheader, 1, sizeof(DataBlockHeader_T), fp)) != sizeof(DataBlockHeader_T)) {
...
#ifdef SECURE_BOOT_ENABLE
				db_offset += secure_offset;
#endif
				ret = fseek(fp, db_offset, SEEK_SET);
				if(ret) {
					ENG_LOG("%s: fseek  fail\n", __FUNCTION__);
					fclose(fp);
					return 0;
				}
...
static int eng_diag_modem_db_read(char *buf, int len, char *rsp)
{
	...
	int fd_ser = -1;
	char modem_file[256] = {0};
	int diag_size = sizeof(MSG_HEAD_T) + sizeof(TOOLS_DIAG_AP_CNF_MD_T) + sizeof(ModemDB_T);
	unsigned char *tmp;
	...
	TOOLS_DIAG_AP_CNF_MD_T *mbrsp = (TOOLS_DIAG_AP_CNF_MD_T *)(tmp + sizeof(MSG_HEAD_T));
	ModemDB_T *filedata = (ModemDB_T *)(tmp + sizeof(MSG_HEAD_T) + sizeof(TOOLS_DIAG_AP_CNF_MD_T));
	...
	do {
			...
			sha1_flag?(file_size = dbheader.length - SHA1_SIZE):(file_size = dbheader.length);//modem.db size(without sha1 size)
			...
			sha1_flag?(db_offset = dbheader.offset + SHA1_SIZE):(db_offset = dbheader.offset);
#ifdef SECURE_BOOT_ENABLE
			db_offset += secure_offset;
#endif
			ret = fseek(fp, db_offset, SEEK_SET);
			if(ret) {
				...
				if(filedata->total - filedata->sn == 1) {
					read_len = file_size - MAX_DIAG_TRANSMIT_MODEMDB_LEN*(filedata->total-1);
				} else {
					read_len = MAX_DIAG_TRANSMIT_MODEMDB_LEN;
                                ...
                                filedata->sn += 1;
                                mbrsp->length = sizeof(ModemDB_T) - MAX_DIAG_TRANSMIT_MODEMDB_LEN + read_size;
                                mbrsp->status = 0;
                                header->len = sizeof(MSG_HEAD_T) + sizeof(TOOLS_DIAG_AP_CNF_MD_T) + mbrsp->length;
...
/* add read modem.db data */
	...
	case CMD_USER_WRITE_REGISTER:
	    ...
	    memset(eng_diag_buf,0,sizeof(eng_diag_buf));
	    rlen = eng_diag_busmonitor_enable(buf, len, eng_diag_buf, sizeof(eng_diag_buf));
	    eng_diag_len = rlen;
	    eng_diag_write2pc(eng_diag_buf, eng_diag_len,fd);
	    return 0;
	    ...
	    memset(eng_diag_buf,0,sizeof(eng_diag_buf));
	    rlen = eng_diag_busmonitor_disable(buf, len, eng_diag_buf, sizeof(eng_diag_buf));
	    eng_diag_len = rlen;
	    eng_diag_write2pc(eng_diag_buf, eng_diag_len,fd);
	    return 0;
	    ...
	    memset(eng_diag_buf,0,sizeof(eng_diag_buf));
	    rlen = eng_diag_busmonitor_get_chaninfo(buf, len, eng_diag_buf, sizeof(eng_diag_buf));
	    eng_diag_len = rlen;
	    eng_diag_write2pc(eng_diag_buf, eng_diag_len,fd);
	    return 0;
	    ...
	    memset(eng_diag_buf,0,sizeof(eng_diag_buf));
	    rlen = eng_diag_busmonitor_get_rtctime(buf, len, eng_diag_buf, sizeof(eng_diag_buf));
	    eng_diag_len = rlen;
	    eng_diag_write2pc(eng_diag_buf, eng_diag_len,fd);
	    return 0;
	    ...
	    memset(eng_diag_buf,0,sizeof(eng_diag_buf));
	    rlen = eng_diag_busmonitor_get_monitordata(buf, len, eng_diag_buf, sizeof(eng_diag_buf));
	    eng_diag_len = rlen;
	    eng_diag_write2pc(eng_diag_buf, eng_diag_len,fd);
	    return 0;
...
static int is_wifimac_invalid(unsigned char *mac)
{
    return (mac[0] & 1) != 0;
}
        ...
        if((crc1==crc3)&&(crc2==crc4)){
            if((cmd_mask & DIAG_CMD_BTBIT) || (cmd_mask & DIAG_CMD_WIFIBIT)){
                ...
                if((head_ptr->subtype&DIAG_CMD_WIFIBIT)>0) {
                    ...
                    if(is_wifimac_invalid(direct->wifiaddr)) {
                        ENG_LOG("%s: WIFIADDR is invalid\n",__func__);
                        wifimac_invalid_flag = 1;
                    }
                }
                if(!wifimac_invalid_flag) {
...
int is_audio_at_cmd_need_to_handle(char *buf,int len){
    ...
    if ( 0==ret ) {
        ...
        ret = at_tok_nextint(&ptr,&eq_or_tun_type);
		if(ret)
			return 0;
        ...
        for ( i = 0; i < sizeof(at_spenha_cmd_to_handle)/sizeof(int); i += 1 ) {
            ...
            if ( at_spenha_cmd_to_handle[i]==cmd_type ) {
                ...
                if ( GET_AUDIO_ENHA_MODE_COUNT!=cmd_type ) {
                    ret = at_tok_nextint(&ptr,&g_index);
					if(ret)
						return 0;
