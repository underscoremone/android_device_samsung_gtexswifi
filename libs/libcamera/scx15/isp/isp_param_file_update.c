...
uint32_t isp_raw_para_update_from_file(SENSOR_INFO_T *sensor_info_ptr,SENSOR_ID_E sensor_id)
{
	...
	raw_fix_info_ptr = (struct sensor_raw_fix_info *)((uint32_t)raw_fix_info_ptr>>2<<2);
	...
	for(i=0;i<SENSOR_MAP_NUM;i++){
		for(j=0;j<SENSOR_AWB_CALI_NUM;j++){
			...
			if(lnc_data_cnt > 1){
				...
				lnc_buf_alloc = (uint16_t*)((uint32_t)lnc_buf_alloc>>2<<2);
...
/*AES DATA Table*/
	...
	for(i=0;i<SENSOR_AE_TAB_NUM;i++){
		...
		if(aes_data_cnt > 1){
			...
			aes_buf_alloc = (uint32_t*)((uint32_t)aes_buf_alloc>>2<<2);
...
/*AEG DATA Table*/
	...
	for(i=0;i<SENSOR_AE_TAB_NUM;i++){
		...
		if(aeg_data_cnt > 1){
			...
			aeg_buf_alloc = (uint16_t*)((uint32_t)aeg_buf_alloc>>2<<2);
...
/*AEG Weight Table*/
	...
	}
	{
		...
		if(ae_weight_data_cnt > 1){
			...
			ae_weight_buf_alloc = (uint8_t*)((uint32_t)ae_weight_buf_alloc>>2<<2);
...
/*Tune Info Table*/
	search_ptr = file_tmp_buf;
	temp_buf_8 = (uint8_t*)malloc(sizeof(struct sensor_raw_tune_info) + 32);
	if(!temp_buf_8){
	...
	}
	{
		...
		if(tune_info_data_cnt > 1){
			...
			raw_tune_info_ptr = (struct sensor_raw_tune_info*)((uint32_t)raw_tune_info_ptr>>2<<2);
