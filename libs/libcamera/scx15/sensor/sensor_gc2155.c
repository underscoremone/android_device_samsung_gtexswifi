/******************************************************************************
 ** Copyright (c) 
 ** File Name:		sensor_GC2155.c 										  *
 ** Author: 													  *
 ** DATE:															  *
 ** Description:   This file contains driver for sensor GC2155. 
 ** 														 
 ******************************************************************************
...
//#define SENSOR_PRINT LOGE
// #define GC2155_CMCC
 
/**---------------------------------------------------------------------------*
 ** 					Local Function Prototypes							  *
 **---------------------------------------------------------------------------*/
LOCAL uint32_t GC2155_PowerOn(uint32_t power_on);
LOCAL uint32_t set_preview_mode(uint32_t preview_mode);
LOCAL uint32_t GC2155_Identify(uint32_t param);
LOCAL uint32_t GC2155_BeforeSnapshot(uint32_t param);
LOCAL uint32_t GC2155_After_Snapshot(uint32_t param);
...
LOCAL uint32_t set_GC2155_ev(uint32_t level);
LOCAL uint32_t set_GC2155_awb(uint32_t mode);
LOCAL uint32_t set_GC2155_anti_flicker(uint32_t mode);
LOCAL uint32_t set_GC2155_video_mode(uint32_t mode);
LOCAL void GC2155_set_shutter();
...
LOCAL SENSOR_REG_TAB_INFO_T s_GC2155_resolution_Tab_YUV[]=
{
	// COMMON INIT
...
LOCAL SENSOR_IOCTL_FUNC_TAB_T s_GC2155_ioctl_func_tab = 
{
    // Internal 
    PNULL,
    GC2155_PowerOn,
 ...
 **---------------------------------------------------------------------------*/
LOCAL void GC2155_WriteReg( uint8_t  subaddr, uint8_t data )
{
		...
		//cmd[0]	=	subaddr;
		//cmd[1]	=	data;		
		//I2C_WriteCmdArr(GC2155_I2C_ADDR_W, cmd, 2, SENSOR_TRUE);
		Sensor_WriteReg_8bits(subaddr, data);
	#else
		DSENSOR_IICWrite((uint16_t)subaddr, (uint16_t)data);
	#endif

	//SENSOR_TRACE("SENSOR: GC2155_WriteReg reg/value(%x,%x) !!\n", subaddr, data);

}

LOCAL uint8_t GC2155_ReadReg( uint8_t  subaddr)
{
	uint8_t value = 0;
	
	#ifndef	_USE_DSP_I2C_
	//I2C_WriteCmdArrNoStop(GC2155_I2C_ADDR_W, &subaddr, 1,SENSOR_TRUE);
	//I2C_ReadCmd(GC2155_I2C_ADDR_R, &value, SENSOR_TRUE);
    ...
    //SENSOR_TRACE("SENSOR: GC2155_ReadReg reg/value(%x,%x) !!\n", subaddr, value);
...
LOCAL uint32_t GC2155_PowerOn(uint32_t power_on)
{
	...
	//uint32_t reset_width=g_GC2155_yuv_info.reset_pulse_width;
...
//		level  must smaller than 8
/******************************************************************************/
LOCAL uint32_t set_GC2155_anti_flicker(uint32_t param )
{
			shutter_flag = 0;
    switch (param)
    {
...
#else
		SENSOR_TRACE("2155 SENSOR: set_video_mode");	
#endif
    return 0;
}
...
//__align(4) const SENSOR_REG_T GC2155_image_effect_tab[][11]=	
...
LOCAL uint32_t GC2155_After_Snapshot(uint32_t param)
{
	...
	GC2155_WriteReg(0xfe,0x00);
	GC2155_WriteReg(0xb6,0x01);  //AEC open
	SENSOR_PRINT("GC2155_After_Snapshot param = %x \n",param);
	return 0;
    
}

LOCAL uint32_t GC2155_BeforeSnapshot(uint32_t sensor_snapshot_mode)
{
