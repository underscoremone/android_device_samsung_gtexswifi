...
LOCAL uint32_t _HI255_set_work_mode(uint32_t mode);
LOCAL uint32_t _HI255_BeforeSnapshot(uint32_t param);
LOCAL uint32_t _HI255_check_image_format_support(uint32_t param);
LOCAL uint32_t _HI255_pick_out_jpeg_stream(uint32_t param);
LOCAL uint32_t _HI255_after_snapshot(uint32_t param);
...
LOCAL const SENSOR_REG_T HI255_640X480[]=
{
	...
	{0x03, 0x00},
	{0x10, 0x11},//Sub 1/2_Preview1 Mode,VSync Type
...
LOCAL uint32_t HI255_InitExt(uint32_t param)	//wujinyou, 2012.11.14
{
...
LOCAL uint32_t _HI255_PowerOn(uint32_t power_on)
{
	...
	SENSOR_PRINT("SENSOR: _HI255_PowerOn (1:on, 0:off): %d \n", power_on);
...
LOCAL uint32_t _HI255_Identify(uint32_t param)
{
	...
	SENSOR_PRINT("anrry:enter 255_Identify\n");

	for(i = 0; i<2; )
	{
		nLoop = 1000;
		SENSOR_PRINT("anrry:for before hi255\n");

		ret = Sensor_ReadReg(reg[i]);

		SENSOR_PRINT("anrry:for after hi255\n");
		SENSOR_PRINT("anrry: hi255 Read reg0x04 = %x\n",ret);
...
LOCAL uint32_t _HI255_after_snapshot(uint32_t param)
{
	...
	SENSOR_PRINT("terry  _HI255_after_snapshot =%d.\n",param);
