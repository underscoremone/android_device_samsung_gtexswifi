...
uint32_t g_stream_buf_id = 1;  //record the bsm buf id for switch buf. the init buf is 0.
...
uint32_t JPEGENC_Poll_MEA_BSM(uint32_t time, uint32_t buf_len,  jpegenc_callback callback)
{
	uint32_t value;
	uint32_t vsp_time_out_cnt = 0;

	SCI_TRACE_LOW("JPEGENC_Poll_MEA_BSM E.\n");
	...
	while (1)
	{
		...
		if(value & 0x4000){ //for MEA done
			...
			SCI_TRACE_LOW("JPEGENC_Poll_MEA_BSM X.\n");
			return 1;
		}
