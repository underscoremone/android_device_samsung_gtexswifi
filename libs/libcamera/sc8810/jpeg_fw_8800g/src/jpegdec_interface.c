...
PUBLIC JPEG_RET_E JpegDec_FwReadoutDecInfo(uint32 *bitstrm_byte_offset_ptr, uint32 bitstrm_bfr_switch_cnt)
{
	...
	READ_REG_POLL(VSP_DBK_REG_BASE+DBK_VDB_BUF_ST_OFF, 0x4, 0x4, TIME_OUT_CLK, "polling the jpeg decoding end!");
...
PUBLIC JPEG_RET_E JPEG_HWDecStartMCUSynchro(uint32 num_of_rows, JPEG_DEC_OUTPUT_PARA_T *output_para_ptr)
{
	...
	//polling bsm status
	READ_REG_POLL(VSP_BSM_REG_BASE+BSM_DEBUG_OFF, ((uint32)1<<31), ((uint32)1<<31), TIME_OUT_CLK, "BSM_DEBUG: polling bsm status");
