...
PUBLIC JPEG_RET_E JPEGFW_AdjustQuantTbl_Dec()
{
	...
	if(!jpeg_fw_codec->progressive_mode)
	{
		...
		time_out_flag = READ_REG_POLL(VSP_DCAM_REG_BASE+DCAM_CFG_OFF, (1<<7), (1<<7), TIME_OUT_CLK, "DCAM_CFG: polling dcam clock status");	
		...
		time_out_flag = READ_REG_POLL(VSP_DCAM_REG_BASE+DCAM_CFG_OFF, 0, 0, TIME_OUT_CLK, "DCAM_CFG: polling dcam clock status");
