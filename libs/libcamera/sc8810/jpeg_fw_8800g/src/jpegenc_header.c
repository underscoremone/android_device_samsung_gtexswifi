...
PUBLIC void OutPutRstMarker(void)
{
	...
	READ_REG_POLL(VSP_BSM_REG_BASE+BSM_READY_OFF, V_BIT_0, V_BIT_0, TIME_OUT_CLK, "BSM,Polling the bsm rfifo can read/write");
...
PUBLIC JPEG_RET_E PutAPP0(void)
{
	...
	*/
	if(READ_REG_POLL(VSP_BSM_REG_BASE + BSM_READY_OFF, V_BIT_0, V_BIT_0, TIME_OUT_CLK, "BSM,Polling the bsm rfifo can read/write"))
	{
		return JPEG_FAILED;
	}
...
PUBLIC JPEG_RET_E PutAPP1(APP1_T *app1_t)
{
	...
	if(NULL == app1_t->datetime){
		...
		SCI_TRACE_LOW("wxz: datetime: %s.", app1_t->datetime);
	}
	...
	if((0 == app1_t->Latitude_dd.numerator) && (0 == app1_t->Latitude_mm.numerator) && (0 == app1_t->Latitude_ss.numerator)
		...
		if(NULL == app1_t->gps_process_method){
			...
			SCI_TRACE_LOW("wxz: gps process method: %s.", app1_t->gps_process_method);
		}
		if(NULL == app1_t->gps_date){
			...
			SCI_TRACE_LOW("wxz: gps date: %s.", app1_t->gps_date);
		}
	...
	uint32 i;
	char *tmp_buf = NULL;
	SCI_TRACE_LOW("wxz: PutAPP1 E. des: %d, make: %d, model: %d, copy: %d, datetime: %d, process_method: %d, gps_date: %d.", description_len, make_len, model_len, copyright_len, datetime_len, gps_process_method_len, gps_date_len);	
	...
	SCI_TRACE_LOW("wxz: PutAPP1 XRes: 0x%x.", header_len + IFD_0th_len + description_len + make_len + model_len);	
	JPEGFW_PutW_II(0x011B); //YResolution
	JPEGFW_PutW_II(0x0005);
	JPEGFW_PutBits32_II(0x1, 32);
	JPEGFW_PutBits32_II(header_len + IFD_0th_len + description_len + make_len + model_len + 8, 32);	
	SCI_TRACE_LOW("wxz: PutAPP1 YRes: 0x%x.", header_len + IFD_0th_len + description_len + make_len + model_len + 8);	
	JPEGFW_PutW_II(0x0128); //ResolutionUnit
	JPEGFW_PutW_II(0x0005);
	JPEGFW_PutBits32_II(0x3, 32);
	JPEGFW_PutBits32_II(0x2, 32);	
	SCI_TRACE_LOW("wxz: PutAPP1 ResU");	
	...
	JPEGFW_PutBits32_II(0x1, 32);
	JPEGFW_PutBits32_II(0x2, 32);	
	SCI_TRACE_LOW("wxz: PutAPP1 YCbCr");	
	...
	SCI_TRACE_LOW("wxz: PutAPP1 copyright");	
	...
	SCI_TRACE_LOW("wxz: PutAPP1 Exif IFD");	
	if(0 != IFD_GPS_len){
	...
	SCI_TRACE_LOW("wxz: Next IFD Offset 0th: %d.", header_len + IFD_0th_len + IFD_0th_val_len + IFD_Exif_len + IFD_Exif_val_len
	...
	SCI_TRACE_LOW("wxz: PutAPP1 Config.");	
	...
	JPEGFW_PutC('0');
	JPEGFW_PutC('0');	
	SCI_TRACE_LOW("wxz: PutAPP1 Flash Ver.");	
	...
	SCI_TRACE_LOW("wxz: iPutAPP1 thumb_height.");	
	JPEGFW_PutBits32_II(0x0, 32); //wxz: ????
	...
	SCI_TRACE_LOW("wxz: PutAPP1 latitude.");	
	...
	SCI_TRACE_LOW("wxz: PutAPP1 longitude.");	
	...
	SCI_TRACE_LOW("wxz: PutAPP1 method.");	
	...
	SCI_TRACE_LOW("wxz: PutAPP1 datestamp.");
	JPEGFW_PutBits32_II(0x0, 32); //wxz: ???	
	...
	JPEGFW_PutBits32_II(0x1, 32);	
	SCI_TRACE_LOW("wxz: process method.");	
	if(NULL != app1_t->gps_process_method){
	...
	if(NULL != app1_t->gps_date){
		...
		JPEGFW_PutC(0x0);	
		SCI_TRACE_LOW("wxz: PutAPP1 GPS.");		
	}
	SCI_TRACE_LOW("wxz: Next IFD Offset Exf: %d.", header_len + IFD_0th_len + IFD_0th_val_len + IFD_Exif_len + IFD_Exif_val_len
		+ IFD_GPS_len + IFD_GPS_val_len);
	...
	JPEGFW_PutBits32_II(0x48, 32); //YResolution Value
	JPEGFW_PutBits32_II(0x1, 32);
	SCI_TRACE_LOW("wxz: PutAPP1 XRes val.");	
