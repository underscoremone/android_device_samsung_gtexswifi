...
#define JPEG_PRINT_LOW(_x_)               	LOGE _x_

/**---------------------------------------------------------------------------*
 **                         Structures                                      *
 **---------------------------------------------------------------------------*/
//#define EXIFW_ALLOC 	SCI_ALLOCA
//#define EXIFW_FREE  	SCI_FREE

#define JPEG_ALIGN_4(_input) ((((_input) + 3) >> 2) << 2)

//#define EXIF_IS_ALL_INVALID(_valid)
...
LOCAL uint32 _GetAsciiStringSize(const uint8 *string_ptr)
{
    ...
    //SCI_PASSERT(PNULL != string_ptr, ("[_GetAsciiStringSize] invalid string pointer"));    /*assert verified*/
							        ...
							        JINF_WEXIF_OUT_PARAM_T *out_param_ptr)
{
    ...
    SCI_PASSERT(in_param_ptr, ("JPEG_AddExif: invalid info_ptr"));    /*assert verified*/
    SCI_PASSERT(in_param_ptr->exif_info_ptr, ("JPEG_AddExif: invalid exif_info_ptr"));    /*assert verified*/
    SCI_PASSERT(in_param_ptr->src_jpeg_buf_ptr, ("JPEG_AddExif: invalid src_jpeg_buf"));    /*assert verified*/
    SCI_PASSERT(in_param_ptr->temp_buf_ptr, ("JPEG_AddExif: invalid temp_buf"));    /*assert verified*/

	JPEG_PRINT_LOW(("[EXIFW_AddExif] src buf = 0x%x, size = %d, thumb  buf = 0x%x, size = %d,\
						temp buf = 0x%x, size = %d, write file = 0x%x",
						in_param_ptr->src_jpeg_buf_ptr, in_param_ptr->src_jpeg_size,
						in_param_ptr->thumbnail_buf_ptr, in_param_ptr->thumbnail_buf_size,
						in_param_ptr->temp_buf_ptr, in_param_ptr->temp_buf_size,
						in_param_ptr->wrtie_file_func));
    ...
    {
    	SCI_PASSERT(in_param_ptr->target_buf_ptr, ("JPEG_AddExif: invalide target buffer"));    /*assert verified*/
										  ...
										  EXIF_ASCII_T *date_time_ptr, uint32 date_time_size)
{
	...
	sprintf((int8 *)date_time_ptr, "%04d:%02d:%02d %02d:%02d:%02d", date_ptr->year, date_ptr->month, date_ptr->day,
