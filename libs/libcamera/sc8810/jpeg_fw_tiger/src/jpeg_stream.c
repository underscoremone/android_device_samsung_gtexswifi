...
PUBLIC uint32 Jpeg_GetStreamPos(JPEG_READ_STREAM_CONTEXT_T *context_ptr)
{
   ...
   if (NULL != context_ptr->read_file_func && NULL != context_ptr->stream_buf) 
   {
       ...
       if ((uint32)context_ptr->stream_ptr >= (uint32)context_ptr->stream_buf)
       {
           offset_in_buf = (uint32)context_ptr->stream_ptr - (uint32)context_ptr->stream_buf;
   ...
   {
       if ((uint32)context_ptr->stream_ptr >= (uint32)context_ptr->stream_buf
          && (uint32)context_ptr->stream_ptr <= (uint32)context_ptr->stream_buf + context_ptr->stream_buf_size)
       {
            offset = (uint32)context_ptr->stream_ptr - (uint32)context_ptr->stream_buf;
       }
       else
       {
...
PUBLIC void Jpeg_SetStreamPos(JPEG_READ_STREAM_CONTEXT_T *context_ptr, uint32 offset)
{
   if (NULL != context_ptr->read_file_func && NULL != context_ptr->stream_buf) 
   {
	   ...
	   if ((uint32)context_ptr->stream_ptr > (uint32)context_ptr->stream_buf + context_ptr->stream_buf_size)
	   {
...
LOCAL BOOLEAN Jpeg_WriteStreamToFile(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr)
{
	...
	uint32 i = 0;
        ...
        if ((uint32)context_ptr->write_ptr > (uint32)context_ptr->write_buf)
        {
            write_size = (uint32)context_ptr->write_ptr - (uint32)context_ptr->write_buf;
        }
...
PUBLIC uint32 Jpeg_GetWritePos(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr)
{
    ...
    if (((uint32)write_ptr >= (uint32)write_buf) && ((uint32)write_ptr <= (uint32)write_buf + write_buf_size))
    {
        write_pos = ((uint32)write_ptr - (uint32)write_buf);
    }
...
PUBLIC BOOLEAN Jpeg_WriteC(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr, uint8 c)
{
    ...
    if ((uint32)write_ptr >= ((uint32)write_buf + write_buf_size))
    {
        if (!Jpeg_WriteStreamToFile(context_ptr))
        {
            return FALSE;
    ...
    if (NULL == write_ptr || (((uint32)write_ptr + 1) > ((uint32)write_buf + write_buf_size)))
    {
        return FALSE;
    }
    else
...
PUBLIC BOOLEAN Jpeg_WriteL(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr, uint32 l)
{
    ...
    if (NULL == write_ptr || (((uint32)write_ptr + 4) > ((uint32)write_buf + write_size)))
    {
        return FALSE;
    }
    else
